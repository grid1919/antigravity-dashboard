/**
 * HTTP Client with HTTPS → HTTP Fallback
 * 
 * Automatically tries HTTP when HTTPS request fails (e.g., certificate issues)
 * Communication is strictly localhost, so this is safe.
 */

import * as https from 'https';
import * as http from 'http';
import type { Protocol, HttpRequestOptions, HttpResponse } from './types';
...
export async function httpRequest<T>(options: HttpRequestOptions): Promise<HttpResponse<T>> {
  ...
  try {
    ...
    if (allowFallback) {
      console.log(`[httpClient] HTTPS failed for ${hostname}:${port}, trying HTTP fallback...`);
      try {
        const result = await doRequest<T>(options, 'http');
        // HTTP succeeded, cache protocol
        setCachedProtocol(hostname, port, 'http');
        console.log(`[httpClient] HTTP fallback succeeded for ${hostname}:${port}`);
        return result;
      } catch {
